...
import com.flight.exception.FlightNotAvailableExceptionone;

@Service//update the database in my sql server
...
//constructor
	...
	public Flight updateFlight(Flight flight, long flightId) {
		  Flight existingflight=flightdao.findById(flightId).orElseThrow( ()->
	        new FlightNotAvailableExceptionone("Booking","Id",flightId));
	...
	public void deleteFlight(long fligthId) {
		 flightdao.findById(fligthId).orElseThrow( ()->
	        new FlightNotAvailableExceptionone("Flight","Id",fligthId));
